...
function clientIdentity(c){return String(c?.id??c?.matricula??c?.codigo_cti??'')}
function clients(){
 normalize();
 const out=[],idx=new Map();
 for(const src of [S.clientes||[],S.custom.clientes||[]])for(const item of src){
  const k=clientIdentity(item);if(!k){out.push(item);continue}
  if(!idx.has(k)){idx.set(k,out.length);out.push(item);continue}
  const i=idx.get(k),base=out[i]||{};out[i]={...base,...item,nome:item?.nome||base?.nome||'',matricula:item?.matricula||base?.matricula||'',codigo_cti:item?.codigo_cti||base?.codigo_cti||''};
 }
 return out
}
...
function clientBy(id){const sid=String(id);return clients().find(x=>String(x.id)===sid||String(x.matricula||'')===sid||String(x.codigo_cti||'')===sid)}
...
function render(){
 ...
 osTable.innerHTML=os.filter(o=>(!st||situation(o)===st)&&[o.numero_os,clientName(o.cliente_id),...(legacyEquipLinks(o.id).map(x=>equipName(x.equipamento_id))),o.defeito_reclamado].join(' ').toLowerCase().includes(q)).sort((a,b)=>num(b.numero_os)-num(a.numero_os)).slice(0,500).map(o=>{let eq=(o.equipamentos||legacyEquipLinks(o.id)).map(x=>equipName(x.equipamento_id)).filter(Boolean).join(', ');return `<tr class="click" onclick="openOS('${esc(o.id)}')"><td><b>${esc(o.numero_os||o.id)}</b></td><td>${esc(o.data_entrada||'')}</td><td><b>${esc(clientName(o.cliente_id))}</b><br><span class="muted">${esc(eq)}</span></td><td>${badge(situation(o))}</td><td>${money(o.total)}</td></tr>`}).join('');
 q=clientSearch.value.toLowerCase();
 clientTable.innerHTML=clients().filter(c=>[c.nome,c.documento,c.cpf_cnpj,c.cidade].join(' ').toLowerCase().includes(q)).slice(0,500).map(c=>{let es=equips().filter(e=>String(equipClient(e))===String(c.id)),co=os.filter(o=>String(o.cliente_id)===String(c.id)),last=co.slice().sort((a,b)=>num(b.numero_os)-num(a.numero_os))[0];return `<tr class="click" onclick="openClient('${esc(c.id)}')"><td><b>${esc(c.nome)}</b><br><span class="muted">${esc(c.cidade||'')}</span></td><td>${es.length}<br><span class="muted">${esc(es.slice(0,3).map(e=>equipName(e.id)).join(', '))}</span></td><td>${co.length}</td><td>${last?`OS ${esc(last.numero_os||last.id)} · ${esc(last.data_entrada||'')}`:''}</td></tr>`}).join('');
 ...
 stockTable.innerHTML=activeProducts().filter(p=>{let gt=(p.codigo_barras||'SEM GTIN').toLowerCase();return (!fpc||productCode(p).toLowerCase().includes(fpc))&&(!fic||productMakerCode(p).toLowerCase().includes(fic))&&(!fgt||gt.includes(fgt))&&(!fn||(p.descricao||'').toLowerCase().includes(fn))&&(!fcat||(p.categoria||'').toLowerCase().includes(fcat))&&(!floc||(p.localizacao||'').toLowerCase().includes(floc))&&(stockFilter.value!=='low'||(num(p.estoque_minimo)>0&&num(p.estoque_atual)<num(p.estoque_minimo)))}).slice(0,900).map(p=>`<tr class="click" onclick="openProduct('${esc(p.id)}')"><td>${p.imagem?`<img class="prodImg" src="${p.imagem}">`:''}</td><td><b>${esc(productCode(p))}</b><br><span class="muted">${esc(productMakerCode(p))}</span></td><td><b>${esc(p.descricao)}</b><br><span class="muted">${esc(p.codigo_barras||'SEM GTIN')}</span></td><td>${num(p.estoque_atual)}</td><td>${num(p.estoque_minimo)}</td><td>${num(p.estoque_maximo)}</td><td>${money(p.valor_venda)}</td><td>${esc(p.categoria||'')}</td><td>${esc(p.localizacao||'')}</td></tr>`).join('');
}