...
async function releaseHtml(resp){
  if(!resp||!resp.ok)return resp;
  let text=await resp.text();
  text=text.replaceAll('0.9.5','0.9.6');
  text=text.replace(
    '<p id="updateStatus" class="muted"></p>',
    '<p id="updateStatus" class="muted">Versão instalada: 0.9.6 — atualização automática ativa.</p>'
  );
  return new Response(text,{status:resp.status,statusText:resp.statusText,headers:{'Content-Type':'text/html; charset=utf-8','Cache-Control':'no-store, no-cache, must-revalidate'}});
}
  ...
  if(req.mode==='navigate' || (url.origin===location.origin && url.pathname.endsWith('/index.html'))){
    event.respondWith((async()=>{
      try{
        ...
        const released=await releaseHtml(network);
